...
struct VocabWord: Identifiable, Codable, Equatable {
    ...
    enum CodingKeys: String, CodingKey {
        case kanji, furigana, meaning, category, level
    }
}
...
class DataLoader {
    ...
    func getWords(for level: Int, category: String? = nil) -> [VocabWord] {
        return allVocab.filter { word in
            let matchesLevel = word.level == level
...
class GauntletManager: ObservableObject {
    ...
    func startGauntlet(level: Int = 5) {
        currentRound = 1
        cumulativePool = []
        score = 0
        multiplier = 1.0
        ...
        let words = DataLoader.shared.getWords(for: level)
        ...
        if words.isEmpty {
            ...
            availableVocab = [
                VocabWord(kanji: "朝", furigana: "あさ", meaning: "Morning", category: "Time", level: 5),
                VocabWord(kanji: "先生", furigana: "せんせい", meaning: "Teacher", category: "People", level: 5),
                VocabWord(kanji: "食べる", furigana: "たべる", meaning: "To eat", category: "Verbs", level: 5),
                VocabWord(kanji: "水", furigana: "みず", meaning: "Water", category: "Nature", level: 5),
                VocabWord(kanji: "赤い", furigana: "あかい", meaning: "Red", category: "Adjectives", level: 5)
            ]
        } else {
...
struct MainTabView: View {
    ...
    init() {
        ...
        navAppearance.backgroundColor = UIColor(Color.synthBackground)
...
// MARK: - The Hunt Selection
struct GauntletSelectionView: View {
    @EnvironmentObject var gauntlet: GauntletManager
    
    // Configuration for the descending difficulty aesthetic (No heavy glows needed!)
    let levels = [
                    ...
                    ScrollView {
                        VStack(spacing: 24) {
                            ForEach(levels, id: \.level) { config in
                                Button(action: {
                                    withAnimation { gauntlet.startGauntlet(level: config.level) }
                                }) {
...
struct FlashcardReviewView: View {
    ...
    var currentWord: VocabWord {
        if gauntlet.activePool.isEmpty { return VocabWord(kanji: "", furigana: "エラー", meaning: "Error", category: "", level: 0) }
    ...
    var body: some View {
            ...
            ZStack {
                ...
                if !isFlipped {
                    ...
                    VStack(spacing: 20) {
                        ...
                        Button(action: { SpeechManager.shared.speak(text: currentWord.furigana, language: "ja-JP") }) {
        ...
        .onAppear {
            if autoPlayAudio && !isFlipped { SpeechManager.shared.speak(text: currentWord.furigana, language: "ja-JP") }
        }
        .onChange(of: isFlipped) {
            if autoPlayAudio {
                SpeechManager.shared.speak(text: isFlipped ? currentWord.meaning : currentWord.furigana, language: isFlipped ? "en-US" : "ja-JP")
            }
        }
...
struct GameBoardView: View {
    ...
    func generateBoard() {
        ...
        while !allPlaced {
            ...
            for word in gauntlet.currentGridWords {
                let chars = Array(word.furigana).map { String($0) }
                var placed = false
                var attempts = 0
    ...
    func checkSelectedWord() {
        ...
        if let match = gauntlet.currentGridWords.first(where: {
            ...
            (selectedChars == $0.furigana || selectedChars == String($0.furigana.reversed()))
        }) {
...
struct BossFightQuizView: View {
    ...
    func generateOptions(for word: VocabWord) -> [String] {
        ...
        if pool.isEmpty {
            pool = [
                VocabWord(kanji: "", furigana: "", meaning: "Fake Answer 1", category: "", level: 0),
                VocabWord(kanji: "", furigana: "", meaning: "Fake Answer 2", category: "", level: 0),
                VocabWord(kanji: "", furigana: "", meaning: "Fake Answer 3", category: "", level: 0)
            ]
        }
