...
class RetroAudioEngine: NSObject {
    ...
    private func precompileSounds() {
        ...
        // Array of 5 players prevents clipping entirely during rapid word selection
...
class GauntletManager: ObservableObject {
    ...
    // Dynamic run progression based on chosen packet size
    ...
    func startCustomGauntlet(words: [VocabWord]) {
        ...
        // Dynamically set total rounds based on 5 nodes per round
...
// MARK: - Custom Themed Alert Component
struct CyberAlertView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 24) {
                ...
                Button(action: {
                    ...
                    Text(buttonText)
                        ...
                        .cornerRadius(8)
                        .shadow(color: .synthCyan.opacity(0.5), radius: 10)
                }
                .padding(.top, 10)
            }
            .padding(30)
        ...
        .zIndex(100) // Ensure it draws over everything else
    }
}
        ...
        .onReceive(timer) { _ in
            ...
            if Double.random(in: 0...1) < (severity * 0.25) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.08) {
                    offsetX = 0; opacity = 1.0; showAberration = false
                }
            }
        }
    }
...
// MARK: - Data Packet Builder (Horizontal Fanning UX)
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // Horizontal Fanning Content Area
                GeometryReader { geo in
                    HStack(spacing: 0) {
                            ...
                            VStack(spacing: 16) {
                                ForEach(categories, id: \.self) { cat in
                                    Button(action: {
                                        ...
                                        subcategories = DataLoader.shared.getSubcategories(for: level, category: cat)
                                        RetroAudioEngine.shared.playWhoosh()
                                        withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) { viewDepth = 1 }
                                    }) {
                                        HStack {
                                        ...
                                        .padding(20).background(Color.synthCard).cornerRadius(8)
                                        .overlay(RoundedRectangle(cornerRadius: 8).stroke(config.color.opacity(0.5), lineWidth: 1))
                                    }
                                }
                            }.padding(24)
                        }
                            ...
                            VStack(spacing: 16) {
                                ForEach(subcategories, id: \.self) { sub in
                                    Button(action: {
                                        ...
                                        words = DataLoader.shared.getWords(for: level, category: selectedCategory, subcategory: sub)
                                        RetroAudioEngine.shared.playWhoosh()
                                        withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) { viewDepth = 2 }
                                    }) {
                                        HStack {
                                ...
                                if !words.isEmpty {
                                    HStack {
                                        ...
                                        Button(action: {
                                            ...
                                            .foregroundColor(allSelected ? .synthPink : config.color)
                                            .padding(.vertical, 8).padding(.horizontal, 12)
                                            .background(Color.synthCard).cornerRadius(6)
                                            .overlay(RoundedRectangle(cornerRadius: 6).stroke((allSelected ? Color.synthPink : config.color).opacity(0.5), lineWidth: 1))
                                        }
                                        Spacer()
                                    }
                                    .padding(.bottom, 4)
                                        ...
                                        HStack {
                                            VStack(alignment: .leading, spacing: 4) {
                                                Text(word.kanji.isEmpty ? word.hiragana : "\(word.kanji) (\(word.hiragana))")
                                                    ...
                                                    .foregroundColor(isSelected ? .synthBackground : .white)
                                                Text(word.meaning)
                                                    .font(.caption.weight(.medium)).fontDesign(.monospaced)
                                                    .foregroundColor(isSelected ? .synthBackground.opacity(0.8) : .gray)
                                            }
                                            Spacer()
                                            if isSelected {
                                                Image(systemName: "checkmark.square.fill").font(.title2).foregroundColor(.synthBackground)
                                            } else {
                                                Image(systemName: "square.dashed").font(.title2).foregroundColor(config.color.opacity(0.5))
                                            }
                                        }
                                        .padding(16).background(isSelected ? config.color : Color.synthCard).cornerRadius(8)
                                        .overlay(RoundedRectangle(cornerRadius: 8).stroke(config.color.opacity(isSelected ? 1.0 : 0.3), lineWidth: isSelected ? 2 : 1))
                                    }
                                }
                            }.padding(24)
                        }
                ...
                // BOTTOM CONSOLE: Data Packet Status & Start Button
                VStack(spacing: 16) {
                    HStack {
                        Image(systemName: isPacketReady ? "externaldrive.fill.badge.checkmark" : "externaldrive.badge.plus")
                            .font(.system(size: 28))
                            .foregroundColor(isPacketReady ? .synthPink : config.color)
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack(spacing: 8) {
                                ...
                                Button(action: {
                                    RetroAudioEngine.shared.playSelectionTap()
                                    withAnimation { showWhy20Alert = true }
                                }) {
                            ...
                            Text("[ \(packet.count) / 20 ] NODES")
                                .font(.headline.weight(.black)).fontDesign(.monospaced)
                                .foregroundColor(packet.count == 20 ? .synthPink : .white)
                            
                            Text("[ \(packet.count / 5) / 4 ] SECTORS CONFIGURED")
                                .font(.caption2.weight(.bold)).fontDesign(.monospaced)
                                .foregroundColor(config.color)
                        }
                        Spacer()
                    }
                    ...
                    if isPacketReady {
                        Button(action: {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                                withAnimation { gauntlet.startCustomGauntlet(words: packet) }
                            }
                        }) {
                            Text("INITIALIZE UPLINK")
                                .font(.title3.weight(.black)).fontDesign(.monospaced)
                                .frame(maxWidth: .infinity).padding().background(Color.synthPink).foregroundColor(.synthBackground).cornerRadius(8).shadow(color: .synthPink.opacity(0.8), radius: 10)
                        }
                        .padding(.horizontal, 24).transition(.move(edge: .bottom).combined(with: .opacity))
                    } else if packet.count > 0 {
                        Text("A sector requires 5 nodes. Select \(5 - (packet.count % 5)) more to configure.")
                            .font(.caption.weight(.bold)).fontDesign(.monospaced)
                            .foregroundColor(.synthYellow).padding(.horizontal, 24)
                    }
                }
                .padding(.top, 16).padding(.bottom, 32).background(Color.synthCard.edgesIgnoringSafeArea(.bottom))
                .overlay(Rectangle().frame(height: 2).foregroundColor(isPacketReady ? .synthPink : config.color), alignment: .top)
            }
            ...
            if showOverloadAlert {
                CyberAlertView(
                    ...
                    message: "Selecting these nodes exceeds the maximum 20-node limit.\n\nYou have [ \(slotsRemaining) ] slots remaining in your data packet.",
            ...
            if showWhy20Alert {
                CyberAlertView(
                    ...
                    message: "Most learners can effectively learn 10–20 new words per day and remember them long-term with regular review. The best approach is to learn fewer words well rather than many words quickly and forget them.\n\n[ ENDLESS RUN MODE : COMING IN FUTURE UPDATE ]",
                    buttonText: "ACKNOWLEDGE",
